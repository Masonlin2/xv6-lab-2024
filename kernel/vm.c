...
// 这里是软件访问页表，不会涉及到中断，但是还是要按照写时复制来判断
int
...
// 增加新的物理地址内存，去除旧映射和权限，添加新的，主要去除COW和增加PTE_W
int 
uvmcowcopy(uint64 va)
{
  ...
  uint64 newpa = (uint64)k_copy((void*)pa);
  ...
  // 不改变原来的权限，因为依旧需要原来的进行是否为写时复制的错误
